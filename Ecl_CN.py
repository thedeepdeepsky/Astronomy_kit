...
class Ecl:
    def __init__(self, tz_hours=8):
        """
        初始化高精度日月食计算引擎
        :param tz_hours: 默认时区偏移（默认 +8 北京时间）
        """
    ...
    def get_global_eclipses(self, start_dt, end_dt):
        ...
        for t, y in zip(t_lunar, y_lunar):
            e_type = ["半影月食", "月偏食", "月全食"][y]
            results.append({
                "Event": "Lunar Eclipse",
                "Type": e_type,
                "Peak_Time": t,
                "Peak_Time_Str": self._fmt_time(t),
                "Details": "月食为全球性事件，背向太阳的半球皆可观测。"
            })
        ...
        for t_phase, phase in zip(t_phases, y_phases):
            if phase == 0:
                ...
                if sep < 1.6:
                    ...
                    # 精确计算 u: 月球本影锥在基准面上的半径(以地球赤道半径为单位)
                    sin_f2 = (self.R_sun - self.R_moon) / d_s
                    L2 = self.R_moon - d_m * sin_f2
                    u = L2 / self.R_earth

                    # Meeus 算法规则细分中心与非中心日食分类
                    mag_approx = None
                    if gamma < 0.9972:
                        if u < 0:
                            e_type = "中心日全食 (Total)"
                        elif u > 0.0047:
                            e_type = "中心日环食 (Annular)"
                        else:
                            omega = 0.00464 * math.sqrt(max(0.0, 1.0 - gamma ** 2))
                            if u < omega:
                                e_type = "全环食 (Annular-Total)"
                            else:
                                e_type = "中心日环食 (Annular)"
                    elif 0.9972 <= gamma < 0.9972 + abs(u):
                        if u < 0:
                            e_type = "非中心日全食 (Non-central Total)"
                        else:
                            e_type = "非中心日环食 (Non-central Annular)"
                    elif gamma < 1.5433 + u:
                        e_type = "日偏食 (Partial)"
    ...
    def get_local_eclipse_details(self, lat, lon, elevation, eclipse_peak_time, is_solar=True):
        ...
        if not is_solar:
            ...
            # Meeus 修正: 采用 Danjon 法国法则替代传统 1/50 放大
            # 相当于视差大约 1.01 倍的膨胀
            ...
            if magnitude >= 1.0:
                ...
                ecl_type = "月偏食 (Partial)"
            else:
                ecl_type = "半影月食 (Penumbral)"
            ...
            return {
                ...
                "Contacts": {
                    "P1 (半影食始)": self._fmt_time(p1_t),
                    "U1 (初亏)": self._fmt_time(u1_t),
                    "U2 (食既)": self._fmt_time(u2_t),
                    "Max (食甚)": self._fmt_time(max_t),
                    "U3 (生光)": self._fmt_time(u3_t),
                    "U4 (复圆)": self._fmt_time(u4_t),
                    "P4 (半影食终)": self._fmt_time(p4_t),
                }
            }
        ...
        if not np.any(penumbra):
            return {"Event": "Solar Eclipse Local", "Visible": False, "Msg": "未进入偏食区。"}
        ...
        if max_sep < abs(rs_max - rm_max):
            ecl_type = "全食 (Total)" if rm_max >= rs_max else "环食 (Annular)"
        else:
            ecl_type = "偏食 (Partial)"
        ...
        app_alt = alt.degrees + self._refraction_correction(alt.degrees)
        if app_alt <= 0.0:
            return {"Event": "Solar Eclipse Local", "Visible": False, "Msg": "发生时太阳位于地平线以下。"}
        ...
        return {
            ...
            "Contacts": {
                "P1 (初亏)": self._fmt_time(p1_t),
                "U1 (食既)": self._fmt_time(u1_t),
                "Max (食甚)": self._fmt_time(max_t),
                "U2 (生光)": self._fmt_time(u2_t),
                "P4 (复圆)": self._fmt_time(p4_t)
            }
        }
...
if __name__ == "__main__":
    ...
    print(f"========== 检索 {start.year} - {end.year} 全球日月食 ==========")
    global_eclipses = ecl.get_global_eclipses(start, end)

    test_solar, test_lunar = None, None

    for ge in global_eclipses:
        print(f"[{ge['Event']}] {ge['Type']} | 时刻: {ge['Peak_Time_Str']}")
        if ge['Event'] == 'Solar Eclipse':
            print(
                f"  -> Gamma值: {ge.get('Gamma_Approx')} | 本影锥半径 u: {ge.get('U_cone_radius')} | (X,Y): ({ge.get('Besselian_X')}, {ge.get('Besselian_Y')})")
            if 'Magnitude_Approx' in ge:
                print(f"  -> 最大食分(理论测算): {ge.get('Magnitude_Approx')}")
    ...
    print(f"\n========== 当地观测详情分析 (测试点: 纬度 {lat}, 经度 {lon}) ==========")

    if test_solar is not None:
        print(f"\n[日食分析波次]: {ecl._fmt_time(test_solar)}")
